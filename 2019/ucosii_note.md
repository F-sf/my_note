...
> 可以更好地实现任务间的协调，虽然运行之会消耗一部分CPU资源，但可以带来更高效的任务协调和同步，从另一层面减小了资源浪费。也可以提供更清晰的程序框架，方便协作交流。

* II和III的区别
  * II中各种操作的参数与III有很多不同，III中要复杂很多。
  * II中任务优先级即为任务ID，同优先级只能存在一个任务，CPU占用为抢占式的；III中同优先级可设置多个任务，初始化每个任务时可为其分配时间片，支持时间片轮转调度同优先级任务。
...
* UCOS中的任务堆栈
  * 任务堆栈是UCOS实现任务调度的关键，其本体是ram中栈外的一块数组，实现与内存栈相似的功能。即像内存中的栈区一样存储运行时的局部变量，并在任务调度发生时将CPU寄存器值入栈，保存当前任务的现场，在重获执行权之后恢复现场。
  * 其大小应根据任务需要使用的局部变量所占空间的大小分配。
* UCOS任务调度
  * 任务调度通过任务及调度OS_Sched()和中断级调度OSIntExit()中调用OS_TASK_SW()实现的。
...
/***** 数据结构部分 *****/
  ...
  typedef struct os_tcb {
      ...
      OSTCBX = prio&0x0F = 7, OSTCBBitX = 1 << OSTCBX = 10000000b */
} OS_TCB;
...
1. 任务堆栈如何保证运行任务内代码时将全局区的任务堆栈对应的内存充当栈区进行使用?
   ...
   >  程序运行时CPU通过SP栈顶指针来确定在内存中何处实现栈结构，UCOS任务调度过程中保存和恢复现场的过程中，涉及到CPU的各个寄存器, 栈顶指针SP, 程序计数器PC。这三个东西可以完全约束CPU当前的运行状态。
...
5. ucosii优先级反转问题的解决是仅在互斥信号量中还是信号量中也有？
